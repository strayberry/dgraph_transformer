...
class Elliptic_Temporal_Dataset():
	...
	def load_node_labels(self, elliptic_args, tar_archive):
		labels = u.load_data_from_tar(elliptic_args.classes_file, tar_archive, replace_unknown=True).long()
		times = u.load_data_from_tar(elliptic_args.times_file, tar_archive, replace_unknown=True).long()
