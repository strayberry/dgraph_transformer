...
def load_data_from_tar(file, tar_archive, replace_unknown=False, starting_line=1, sep=',', type_fn = float, tensor_const = torch.DoubleTensor):
    f = tar_archive.extractfile(file)
    lines = f.read()
    lines=lines.decode('utf-8')
    if replace_unknown is True:
        lines=lines.replace('unknown', '0')
